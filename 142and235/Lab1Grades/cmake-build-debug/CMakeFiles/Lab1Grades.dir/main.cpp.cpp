...
int main(int argc, char* argv[])
{
    ...
    int **letters = new int*[num_students];
    for (int j = 0; j < num_exams; j++)
    {
    ...
    {
        out << setw(22) << students[i] << " ";
        if (static_cast<float>(finalGrades[i]) <= (averages[i] + 5) && static_cast<float>(finalGrades[i]) >= (averages[i] - 5))
        {
            gradeLetter = 'C';
        }
        else if (static_cast<float>(finalGrades[i]) >= averages[i] && static_cast<float>(finalGrades[i]) <= (averages[i] + 15))
        {
            gradeLetter = 'B';
        }
        else if (static_cast<float>(finalGrades[i]) <= averages[i] && static_cast<float>(finalGrades[i]) >= (averages[i] - 15))
        ...
        else if (static_cast<float>(finalGrades[i]) >= (averages[i] + 15))
        {
            gradeLetter = 'A';
        }
        else if (static_cast<float>(finalGrades[i]) <= (averages[i] - 15))
